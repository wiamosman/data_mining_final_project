...
Model,Accuracy,Precision,Recall,F1 Score,Total Time (s)
...
Summary of Findings
    ...
    Transformers (SOTA): XLNet achieved the highest F1 score (0.9339). These models excel at capturing context but require significantly more GPU time.

    Traditional ML (Efficiency): Logistic Regression (LR) and Naive Bayes (NB) are incredibly fast. NB completed the task in under a second, making it the most cost-effective for simple applications.
